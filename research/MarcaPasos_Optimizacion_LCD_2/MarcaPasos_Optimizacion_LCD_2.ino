...
volatile bool enviarPulsoExterno = false;
volatile bool memoriaEnUso = false;
byte tareasPesadas;
byte clock;
...
void polleoMidiInterrupt() {
  if (memoriaEnUso) return;
...
void setup() {
  ...
  MIDI.setHandleClock(funcionInterrupcion);
  Timer1.initialize(1000); // 1000 microsegundos = 1 ms
  Timer1.attachInterrupt(polleoMidiInterrupt);
}
...
void funcionInterrupcion() { 
  ...
  if (MidiProgramming::modeMidiClock == 0) {
    ...
    for(byte i = 0; i < N_MAX_SEQS; i++){
      ...
      midiProg[i].midiNotesOff(); // Apaga notas del paso anterior de forma segura
    }
    MidiProgramming::tipoMsgMidi = 0;
  } 
  
  // ==========================================
  // MODO RELOJ INTERNO (1)
  // ==========================================
  else {
    if (MidiProgramming::modeMidiClock == 1) {
      enviarPulso = true;
    }
  }
}
...
void loop() {
  // Manejo del pulso en Modo Reloj Interno (1)
  if (enviarPulso) {
    enviarPulso = false; // Bajamos la bandera
    
    MIDI.sendRealTime(midi::Clock); // Envía el pulso al exterior
    clock++;
    if(clock > 23){ clock = 0; }
    
    MidiProgramming::tipoMsgMidi = 0xF8;
    
    // 1. Primero avanzamos los punteros de las secuencias
    for(byte i = 0; i < N_MAX_SEQS; i++){
        midiProg[i].midiSeq();
    }
    
    // 2. Ejecutamos la lógica musical respetando si es Nota, CC, si está armado y si hay Play
    for(byte i = 0; i < N_MAX_SEQS; i++){
      if(p->nSequence[i].armed && midiUI.devicePlay){
        if(p->nSequence[i].seqMode == 0){
          midiProg[i].midiNotesOn();
      ...
      midiProg[i].midiNotesOff(); // Manejo idéntico al modo externo para evitar notas colgadas
    }
    
    MidiProgramming::tipoMsgMidi = 0;
    return; // Saltamos el resto del loop en este ciclo para dar prioridad al pulso enviado
  }

  // Comandos de Start/Stop en modo interno
  ...
  // --- El resto de tu loop se mantiene exactamente igual ---
