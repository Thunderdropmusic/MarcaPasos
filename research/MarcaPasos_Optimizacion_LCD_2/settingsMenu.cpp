...
void MenusButtons::selectorClock(){
  ...
  if (rollDerecha) {
    // Cambiar a MODO INTERNO (1)
    if(MidiProgramming::modeMidiClock == 0){ 
      MidiProgramming::modeMidiClock = 1;
      
      // El Timer pasa a mandar: corre a la velocidad del BPM actual
    ...
    // Cambiar a MODO EXTERNO (0)
    if(MidiProgramming::modeMidiClock == 1){
      MidiProgramming::modeMidiClock = 0;
      
      // ¡Ojo! No apagamos el Timer. Lo ponemos a "pollear" el puerto serial
      // de forma ultra rápida (cada 1ms) para ganarle al lag del LCD.
      Timer1.initialize(1000); // 1000 microsegundos = 1 milisegundo
...
void MenusButtons::verSequencia(){
  if (!timeDebounce()) return;
  if (repeatedButton) return;
