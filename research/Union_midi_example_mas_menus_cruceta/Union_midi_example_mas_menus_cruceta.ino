...
const char* subdivisionesArray [11] = {"1/8", "1/4", "1/2", "1/1", "2/1", "4/1", "1/6", "1/3", "1/3", "2/1", "4/1",};
...
void checkButtons() {
  ...
  if (menuActual == 1 && seleccion == 1) { subDivMode ++; }
    if(menuActual == 1 && seleccion == 3){
      if(!repeated && tiempoActual - ultimoTiempoBoton >= 250){   
        if (subdivision > 1){
          subdivision = subdivision >> 1;// 2**1 2**2 2**3
          updateLCD = true;
          repeated = true;
          indiceSubdivisiones--;
          ultimoTiempoBoton = tiempoActual;
...
void drawMenu1() {
  if(seleccion == 1) { 
    lcd.setCursor(3,0), lcd.write(byte(0)), lcd.print("SUBDIV: "), lcd.write(byte(1)), 
    lcd.setCursor(6,1), lcd.print(subdivisionesArray[indiceSubdivisiones]);}
  
  else if(seleccion == 3) { 
    lcd.setCursor(4,0), lcd.print("SUBDIV: "), 
    lcd.setCursor(5,1), lcd.write(byte(0)), lcd.print(subdivisionesArray[indiceSubdivisiones + (6*subDivMode)]),lcd.write(byte(1));}
}
void drawMenu2() {
  lcd.print("BPM = "); 
}
