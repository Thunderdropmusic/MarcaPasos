...
void MidiInterface::checkMutes() {
  ...
  for (int i = 0; i < NUM_MUTES; i++) {
    ...
    if (mute) {
      ...
      if(mutePulsado[0] && mutePulsado[1] && MidiProgramming::modeMidiClock == 1 && !comandoEnviado && tiempoActualMillis - ultimoTiempoBotonMute[0] > 250 && tiempoActualMillis - ultimoTiempoBotonMute[1] > 250){
        MidiProgramming::tipoMsgMidi = 0xFA;
        ultimoTiempoBotonMute[0] = tiempoActualMillis;
        ultimoTiempoBotonMute[1] = tiempoActualMillis;
        comandoEnviado = true;
      }
      else if(mutePulsado[2] && mutePulsado [3] && MidiProgramming::modeMidiClock == 1 && !comandoEnviado && tiempoActualMillis - ultimoTiempoBotonMute[2] > 250 && tiempoActualMillis - ultimoTiempoBotonMute[3] > 250){
        MidiProgramming::tipoMsgMidi = 0xFC;
