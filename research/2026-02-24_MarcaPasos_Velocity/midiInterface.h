  ...
  for (int i = 0; i < numMutes; i++) {
    ...
    if (mute) {
      ...
      */
      ...
      if (mutePulsado[i] == true && !pulsedIndividualOctave && !octPlusPulsed && !octRestPulsed && !poteMovido) {
      ...
      else if(mutePulsado[i] == true && poteMovido){
        mutePulsado[i] = false;
        muteActivado = false;
        poteMovido = false;
      }
...
void checkPotes() {
  for (int i = 0; i < numPotes; i++) {
    ...
    if(muteActivado){
      int val = constrain(map(val, 1018, 30, 0, 127), 0, 127);
      velocityValue[i] = val;
      if (abs(val - velocitySaveValue[i]) > 1){
        velocitySaveValue[i] = val;
        movedPoteValue = val;
        movedPoteNumber = i;
      ...
      int indice = constrain(map(val, 1018, 30, 0, nNotasEscalas[escalaSeleccionada] - 1), 0, nNotasEscalas[escalaSeleccionada] - 1);
