  ...
  */
  ...
  for(int i = 0; i < N_MAX_SEQUENCES; i++){
    for(int j = 0; j < NUM_POTES; j++){
      ...
      globalNote[i][j] = constrain(map(lectura, 1018, 8, 48, 60), 48, 60);
      globalVelocity[i][j] = constrain(map(lectura, 1018, 8, 0, 127), 0, 127);;
    }
  }
...
void loop() {
  if (enviarPulso && midiProg.modeMidiClock == 1) {
    ...
    for(int indiceSecuencia = 0; indiceSecuencia < N_MAX_SEQUENCES; indiceSecuencia++){
      midiProg.midiSeq(indiceSecuencia);
      midiProg.midiNotesOn(indiceSecuencia);
      midiProg.midiNotesOff(indiceSecuencia);
  ...
  if(menusUI.menuActual == 0 && menusUI.seleccion == 5){
    if(midiProg.nStep[presetsUI.indexSequence] != drawUI.nStepAnterior){
