...
inline void DrawMenus::staticScreen1(){
  ...
  lcd.noBlink(), printAt(1, 0, "SEQ  "), lcd.write(126), lcd.print(" ");
...
inline void DrawMenus::drawMenuNotes() {
  ...
  else if(menusUI.seleccion == 6){nScreen = 2;}
  ...
  switch(menusUI.seleccion){
    ...
    case 2: 
      ...
      if(s->subdivMode == 0 || s->subdivMode == 1){
        ...
        printAt(9, 2, subdivisionesComplejasNumChar[s->indComplexSubdivX]), writeAt(5, 2, subdivisionesComplejasDenChar[s->indComplexSubdivY]);
      }
      drawMutes();
      drawSteps();
      break;
...
void DrawMenus::calcularCurvaParaPantalla(byte* visualBuffer, int puntosPorPaso, int subdivision) {
  ...
  for (int step = 0; step < totalSteps; step++) {
    ...
    // 4. Calcular tamaños globales para que la matemática no mienta
