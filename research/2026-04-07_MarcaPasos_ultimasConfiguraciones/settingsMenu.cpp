  ...
  switch(seleccion){
    case 1:
      if(btnAbajo){
        ...
        if(s->escalaSeleccionada > 4){s->escalaSeleccionada = 0;}
        aplicarCambiosEncoder();
      }
      else if(rollIzquierda){
        s->escalaSeleccionada --;
        ...
        if(s->escalaSeleccionada > 4){s->escalaSeleccionada = 5;}
        aplicarCambiosEncoder();
      }
      resetEncoder();
      break;
    case 2:
      if(btnArriba){
        ...
        s->noteTone++;
        if(s->escalaSeleccionada > 11){s->escalaSeleccionada = 0;}
        for(int i = 0; i < N_MAX_SEQS; i++){
        ...
        s->noteTone--;
        if(s->escalaSeleccionada > 11){s->escalaSeleccionada = 5;}
        aplicarCambiosEncoder();
        for(int i = 0; i < N_MAX_SEQS; i++){
          p->nSequence[i].noteTone = s->noteTone;
        }
