...
void MidiPresets::saveSeqSD(char* nombreBase, byte tipoGuardado){
  ...
  if(tipoGuardado == 0){
    ...
    miArchivo.write((const uint8_t *)seqActiva, sizeof(Sequence));
  } else {
...
void MidiPresets::loadSeqSD(char* nombreBase, byte tipoCarga){
  ...
  if(!sd.exists(rutaArchivo)){
    Serial.print("Error: No existe el archivo ");
  ...
  if(tipoCarga == 0){
    Sequence* seqActiva = getActiveSequence(); 
    miArchivo.read((uint8_t *)seqActiva, sizeof(Sequence));
  } 
  else {
  ...
  // 7. ¡VITAL! Le decimos a la pantalla que redibuje todo con los datos nuevos
  drawUI.updateLCD = true;
