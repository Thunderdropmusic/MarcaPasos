...
void MidiInterface::checkMutes() {
  ...
  bool seqLayerButton = !digitalRead(seqLayerChanger);
  if(seqLayerButton){
    seqLayerPulsed = true;
    if(seqLayerPulsed && timeDebounce(ultimoTiempoBotonSeqLayer, 500)){
      if(seqLayer == 0){seqLayer = 4;}
      else if(seqLayer == 4){seqLayer = 0;}
      ultimoTiempoBotonSeqLayer = tiempoActualMillis;
    }
  }
  
  for(int i = seqLayer; i < 4+seqLayer; i++){
    if(p->nSequence[i].armed){
      digitalWrite(seqPinLed[i-seqLayer], HIGH);
    }
    else if(!p->nSequence[i].armed){
      digitalWrite(seqPinLed[i-seqLayer], LOW);
    }
  }
  ...
  for(int i = 0; i < 4; i++){
    if(seqButton[i]){
      ...
      if(seqArmPulsed[i] && timeDebounce(ultimoTiempoBotonArm, 500)){
        if(!p->nSequence[i].armed){
          if(MidiProgramming::modeMidiClock == 0){flagArmed[i+seqLayer] = true;}
