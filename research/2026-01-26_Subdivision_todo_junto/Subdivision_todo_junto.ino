...
const char* subdivisionesArray [12] = {"12/1","1/8", "1/6", "1/4", "1/3","1/2", "3/2", "1/1", "3/4", "2/1", "3/8", "4/1"};
int indiceSubdivisiones = 0;
int subDivMode = 0;
int multiplicadorSubdivision = 1;
...
void checkButtons() {
  ...
  if (digitalRead(pinButton5) == HIGH) { // Izquierda
    if (menuActual == 1 && seleccion == 1) { /*subDivMode ++;*/ }
    else if (menuActual == 1 && seleccion == 3) {
      if (!repeated && tiempoActual - ultimoTiempoBoton >= 250){
        if (subdivision < 96){
          if (subdivision % 3 != 0 && subdivision > 3){
            multiplicadorSubdivision = multiplicadorSubdivision << 1;//3*2^1
          }
          subdivision += multiplicadorSubdivision;
          updateLCD = true;
          repeated = true;
          indiceSubdivisiones++;
  ...
  if (menuActual == 1 && seleccion == 1) { /*subDivMode --; */}
    if(menuActual == 1 && seleccion == 3){
      if(!repeated && tiempoActual - ultimoTiempoBoton >= 250){   
        if (subdivision > 3){
          subdivision -= multiplicadorSubdivision;
          if (subdivision % 3 != 0 && subdivision % 3 != 0 && subdivision > 3){
            multiplicadorSubdivision = multiplicadorSubdivision >> 1;//3*2^1
          }
          updateLCD = true;
          repeated = true;
...
void drawMenu1() {
  if(seleccion == 1) { 
    ...
    lcd.setCursor(6,1), lcd.print(subdivisionesArray[indiceSubdivisiones]);}
  
  else if(seleccion == 3) { 
    lcd.setCursor(4,0), lcd.print("SUBDIV: "), 
    lcd.setCursor(5,1), lcd.write(byte(0)), lcd.print(subdivisionesArray[indiceSubdivisiones]),lcd.write(byte(1));}
}
void drawMenu2() {
  lcd.print("BPM = "); 
}
