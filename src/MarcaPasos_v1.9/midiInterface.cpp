    ...
    if(abs(val - potSaveValue[i]) > 20){
      ...
      if(muteActivado){
        ...
        if(menusUI.putNameFlag){
          byte index = constrain(map(potSaveValue[i], 1018, 30, 0, 35), 0, 35);
          if(index <= 9){
          ...
          int notaEscala = constrain(map(potSaveValue[i], 1018, 30, 0, SEQ.nNotasEscalas[p->escalaSeleccionada] - 1), 0, SEQ.nNotasEscalas[p->escalaSeleccionada] - 1);
          s->steps[i].note = SEQ.escalasNotas[p->escalaSeleccionada][notaEscala] + p->noteTone;
