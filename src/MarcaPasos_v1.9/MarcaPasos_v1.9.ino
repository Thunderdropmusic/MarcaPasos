  ...
  */
  ...
  for(byte i = 0; i < N_MAX_SEQUENCES; i++){
    ...
    for(byte j = 0; j < NUM_POTES; j++){
      ...
      byte notaEscala = constrain(map(lectura, MAX_POTE_VALUE, 30, 0, midiProg[i].nNotasEscalas[p->escalaSeleccionada] - 1), 0, midiProg[i].nNotasEscalas[p->escalaSeleccionada] - 1);
      p->nSequence[i].steps[j].note = midiProg[i].escalasNotas[p->escalaSeleccionada][notaEscala];
