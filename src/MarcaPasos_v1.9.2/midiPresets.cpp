...
void MidiPresets::sdInit(){
  if (!sd.begin(chipSelect, SPI_HALF_SPEED)) {drawUI.sdErrorMsg_init();}
...
void MidiPresets::readPresetsButtons(){
  ...
  for(int i = 0; i < 4; i++){
    if(slots[i] && !savePresetButton){slotLoad(i);}
    else if(slots[i] && savePresetButton){slotSave(i);}
  }
...
void MidiPresets::saveSeqSD(char* nombreBase, byte tipoGuardado){
  ...
  if(tipoGuardado == 0){
    ...
    sprintf(rutaArchivo, "SEQS/%s", nombreArchivo); // /% indica a partir de donde debe escribir
