...
void MidiInterface::checkMutes() {
  ...
  for (int i = 0; i < NUM_MUTES; i++) {
    ...
    if (mute) {
      ...
      if (octPlusPulsed && tiempoActualMillis - ultimoTiempoBotonOct > 500 && globalOctave[i] < 9) {
      ...
      } else if (octRestPulsed && tiempoActualMillis - ultimoTiempoBotonOct > 500 && globalOctave[i] > 0) {
        ultimoTiempoBotonOct = tiempoActualMillis;
        timeShowOctValue = tiempoActualMillis;
        globalOctave[i]--;
        menusUI.menuActual = 4;
  ...
  if (!muteActivado && octPlusPulsed && tiempoActualMillis - ultimoTiempoBotonOct > 500) {
    ...
    timeShowOctValue = tiempoActualMillis;
    for (int i = 0; i < NUM_MUTES; i++) {
      if(globalOctave[i] < 9){globalOctave[i]++;}
    }
    ...
    timeShowOctValue = tiempoActualMillis;
    for (int i = 0; i < NUM_MUTES; i++) {
      if(globalOctave[i] > 0){globalOctave[i]--;}
    }
    menusUI.menuActual = 5;
    drawUI.updateLCD = true;
  }
...
void MidiInterface::checkPotes() {
  for (int i = 0; i < NUM_POTES; i++) {
    ...
    if(abs(val - potSaveValue[i]) > 20){
      ...
      if(muteActivado){
        globalVelocity[i] = constrain(map(potSaveValue[i], 1018, 30, 0, 127), 0, 127);
        movedPoteValue = globalVelocity[i];
        movedPoteNumber = i;
        ...
        int notaEscala = constrain(map(potSaveValue[i], 1018, 30, 0, midiProg.nNotasEscalas[globalEscalaSeleccionada] - 1), 0, midiProg.nNotasEscalas[globalEscalaSeleccionada] - 1);
        globalNote[i] = midiProg.escalasNotas[globalEscalaSeleccionada][notaEscala];
        movedPoteValue = globalNote[i];
        movedPoteNumber = i;
        timeShowPotValue = tiempoActualMillis;
        menusUI.menuActual = 3;
        drawUI.updateLCD = true;
