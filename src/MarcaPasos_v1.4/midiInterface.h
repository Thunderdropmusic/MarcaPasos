  ...
  for (int i = 0; i < numPotes; i++) {
    ...
    if(abs(val - potSaveValue[i]) > 20){
      ...
      if(muteActivado){
        ...
        int notaEscala = constrain(map(potSaveValue[i], 1018, 30, 0, nNotasEscalas[escalaSeleccionada] - 1), 0, nNotasEscalas[escalaSeleccionada] - 1);
        notesValue[i] = escalasNotas[escalaSeleccionada][notaEscala];
