...
const char* charEscalasNotas[13] = {"C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B", "C"};
...
inline void update_LCD(){
  ...
  else if (menuActual == 4) drawScreenOctavas();
  else if (menuActual == 5) drawScreenTodasOctavas();
  updateLCD = false; 
}
...
inline void drawMenuPrincipal() {
  ...
  switch(seleccion){
    ...
    case 5: 
      ...
      if (!mutesValue[indiceMovedMute] && indiceMovedMute <= nTotalSteps - 1) {lcd.write(byte(6));}
      else if(mutesValue[indiceMovedMute] && indiceMovedMute <= nTotalSteps - 1){lcd.write(byte(5));}
      drawSteps();
    break;
  }
}
...
inline void drawScreenPotes() {
  ...
  if(muteActivado){
    ...
    lcd.print(charEscalasNotas[notesValue[int(movedPoteNumber)]]);
...
inline void drawScreenTodasOctavas() {
  ...
  lcd.clear();
  for(int i = 0; i < 4; i++){
    lcd.print(charEscalasNotas[notesValue[i]]);
    lcd.print(int(movedPoteValue/12) + octavaValue[i]);
    lcd.print(" ");
  }
  lcd.setCursor(0,1);
  for(int i = 4; i < numMutes; i++){
    lcd.print(charEscalasNotas[notesValue[i]]);
    lcd.print(int(movedPoteValue/12) + octavaValue[i]);
    lcd.print(" ");
  }
...
inline void drawSteps() {
  if(lastNTotalSteps != nTotalSteps){
    ...
    if (!mutesValue[nTotalSteps - 1]) {
      if (nTotalSteps == nStep) {
        lcd.write(byte(7));
      }
      else {
        lcd.write(byte(6));
