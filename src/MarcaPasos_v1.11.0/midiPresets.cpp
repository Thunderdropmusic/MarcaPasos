...
bool MidiPresets::receiveSeqSDDirecto() {
  drawUI.lcd.clear();
  drawUI.printAt(2, 1, "RECIBIENDO...");

  // Si por algún motivo el byte 0xAA (CMD_START_TRANSFER) sigue en el buffer, lo purgamos
  if (Serial.available() > 0 && Serial.peek() == CMD_START_TRANSFER) {
    Serial.read();
  }

  auto readByteTimeout = []() -> int {
  ...
  // 1. Leer tipo de guardado (0 = SEQ, 1 = PAT)
  int tipoVal = readByteTimeout();
  if (tipoVal == -1) return false;
  byte tipoGuardado = (byte)tipoVal;

  // 2. Leer nombre del archivo (8 caracteres)
  char nombreBase[9];
  for (int i = 0; i < 8; i++) {
    int charVal = readByteTimeout();
    if (charVal == -1) return false;
    nombreBase[i] = (char)charVal;
  }
  nombreBase[8] = '\0';

  // 3. Leer tamaño (2 bytes)
  int sizeH = readByteTimeout();
  int sizeL = readByteTimeout();
  if (sizeH == -1 || sizeL == -1) return false;
  uint16_t tamanoArchivo = (uint16_t)((sizeH << 8) | sizeL);

  // Responder ACK al PC de que la cabecera es correcta
  Serial.write(CMD_ACK);
  Serial.flush();

  // 4. Montar la ruta exacta igual que hace saveSeqSD
  char rutaArchivo[32];
  char nombreArchivo[13];
  int longitud = 8;
  while (longitud > 0 && (nombreBase[longitud - 1] == ' ' || nombreBase[longitud - 1] == '-')) {
  ...
  // Abrir con los mismos flags probados de saveSeqSD
  File miArchivo = sd.open(rutaArchivo, O_WRITE | O_CREAT | O_TRUNC);
  if (!miArchivo) {
  ...
  // 5. Recepción por bloques de 32 bytes
  byte bufferCarga[32];
  uint16_t bytesRecibidos = 0;

  while (bytesRecibidos < tamanoArchivo) {
    uint16_t restantes = tamanoArchivo - bytesRecibidos;
    uint16_t aLeer = (restantes > 32) ? 32 : restantes;
    
    uint16_t leidosEnBloque = 0;
    unsigned long tBloque = millis();

    while (leidosEnBloque < aLeer) {
      if (Serial.available() > 0) {
        bufferCarga[leidosEnBloque++] = Serial.read();
        tBloque = millis();
      } else {
        if (millis() - tBloque > 3000) {
          miArchivo.close();
          Serial.write(CMD_ERROR);
          Serial.flush();
          return false;
    ...
    miArchivo.write(bufferCarga, aLeer);
    bytesRecibidos += aLeer;
  }

  miArchivo.sync();
  miArchivo.close();

  // Responder ACK final al PC
  Serial.write(CMD_ACK);
  Serial.flush();
