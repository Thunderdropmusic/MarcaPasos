...
inline void DrawMenus::staticScreenNotes() {
  ...
  if (nScreen != nAnteriorScreen || menusUI.menuAnterior != menusUI.menuActual) {
    printAt(0, 1, "                    "); // Borrado instantáneo enviando 20 espacios de golpe
    printAt(0, 2, "                    ");
    printAt(0, 3, "                    ");
  }
...
inline void DrawMenus::staticScreenCC() {
  if (juegoCaracteresCargado != 2) {
    charactersMenu2();
    juegoCaracteresCargado = 2;
  }

  if (menusUI.menuAnterior != menusUI.menuActual || nScreen != nAnteriorScreen) {
    printAt(0, 1, "                    ");
    printAt(0, 2, "                    ");
    printAt(0, 3, "                    ");
  }
...
inline void DrawMenus::drawCCVisualizer() {
  ...
  for (int i = 0; i < curTotalSteps; i++) {
    int val = constrain(visualBuffer[i], 0, 127);
    int nivel = map(val, 0, 127, 0, 7);
...
void DrawMenus::calcularCurvaParaPantalla(byte* visualBuffer, int nCharStep, int resolution) { 
  ...
  for (int step = 0; step < totalSteps; step++) { 
    ...
    // 1. Buscar punto de origen (hacia atrás con wrap-around)
    int prevStep = step;
    int countBack = 0;
    while (countBack < totalSteps) {
      if (!stp[prevStep].ccMutes) break;
      prevStep--;
      if (prevStep < 0) { prevStep = totalSteps - 1; }
      countBack++;
    }

    // 2. Buscar punto de destino (hacia adelante con wrap-around)
    int nextStep = step + 1;
    if (nextStep >= totalSteps) nextStep = 0;
    int countForward = 0;
    while (countForward < totalSteps) {
      if (!stp[nextStep].ccMutes) break;
      nextStep++;
      if (nextStep >= totalSteps) nextStep = 0;
      countForward++;
    }

    // 3. Obtener valores CC
    int lastValue = stp[prevStep].ccValue;
    int nextValue = stp[nextStep].ccValue;
    int ccStepSize = nextValue - lastValue; 

    // 4. Distancias circulares
    int distanceSteps = (nextStep - prevStep + totalSteps) % totalSteps;
    if (distanceSteps == 0) distanceSteps = totalSteps;

    // 5. Offset del paso actual
    int offsetSteps = (step - prevStep + totalSteps) % totalSteps;

    // 6. Cálculo de resolución con tipos 'long'
    long totalResolution = (long)distanceSteps * (long)resolution; 
    long indexResolution = (long)offsetSteps * (long)resolution; 
    long divisor = (totalResolution > 1) ? totalResolution : 2; 

    // 7. Generar valor para el buffer
    for (int p = 0; p < nCharStep; p++) { 
      long subReal = ((long)p * (long)resolution) / nCharStep; 
      long indexCurva = indexResolution + subReal;

      long valorCurva = midiProg[presetsUI.indexSequence].ccCurveFunction((int)indexCurva, (int)divisor, stp[prevStep].ccSmoothCurve);

      int valorCalculado = lastValue + (((long)ccStepSize * valorCurva) / 255);

      visualBuffer[(step * nCharStep) + p] = constrain(valorCalculado, 0, 127); 
    }
  }
}
