...
void DrawMenus::drawMenuSettings(){
  switch(menusUI.nMenuSettings){
    ...
    case 0: 
      if(menusUI.seleccion == 1 || menusUI.seleccion == 2 || menusUI.seleccion == 3 || menusUI.seleccion == 4){nScreen = 20;}
      else if(menusUI.seleccion == 5 || menusUI.seleccion == 6 || menusUI.seleccion == 7 || menusUI.seleccion == 8){nScreen = 21;}
      else if(menusUI.seleccion == 9){nScreen = 22;}
    ...
    case 1:
      ...
      switch(menusUI.seleccion){
        ...
        case 1:
          ...
          else if(menusUI.whatSaveSelect == 1) {writeAt(4, 1, byte(0)), lcd.print(F("Pattern")), printAt(9,2,".pat");}
        ...
        case 2:
          if(menusUI.menuAnterior != menusUI.menuActual || nScreen != nAnteriorScreen || updateValues){staticScreenSettings();}
          lcd.noBlink();
          if(menusUI.whatSaveSelect == 0) {printAt(5, 1, "Sequence"), lcd.print(F(" ")), printAt(9,2,".seq");}
          else if(menusUI.whatSaveSelect == 1) {printAt(4, 1, " "), lcd.print(F("Pattern")),printAt(9,2,".pat");}
    ...
    case 2:
      ...
      switch(menusUI.seleccion){
        ...
        case 1:
          ...
          else if(menusUI.whatSaveSelect == 1) {writeAt(7, 1, byte(0)), lcd.print(F("Pattern"));}
          break; 
        ...
        case 2:
          ...
          for(int i = 0; i < 4; i++){
            ...
            if((menusUI.scrollOffset + i) < menusUI.maxIndex){
              ...
              if(menusUI.whatSaveSelect == 0) {lcd.print(F(".seq"));}
              else if(menusUI.whatSaveSelect == 1) {lcd.print(F(".pat"));}    
            } 
            else {printAt(1, i, "            ");}
          }
          break;
    ...
    case 3:
      ...
      switch(menusUI.seleccion){
        case 1:
          ...
          else if(menusUI.whatSaveSelect == 1) {writeAt(8, 1, byte(0)), lcd.print(F("Pattern"));}
          break; 
          // PANTALLA DE SELECCIÓN DE ARCHIVO
        case 2:
          if(menusUI.menuAnterior != menusUI.menuActual || nScreen != nAnteriorScreen || updateValues){
        ...
        case 3:
          if(menusUI.menuAnterior != menusUI.menuActual || nScreen != nAnteriorScreen || updateValues){staticScreenSettings();}
          printAt(1, 2, midiUI.charNumber);
          if(!menusUI.deleteConfirm){writeAt(1,3,byte(0)), lcd.print(F("No")), lcd.write(byte(1)), printAt(11,3," "), lcd.print(F("Yes")), lcd.print(F(" "));}
          else if(menusUI.deleteConfirm){printAt(1,3," "), lcd.print(F("No")), lcd.print(F(" ")), writeAt(11,3,byte(0)), lcd.print(F("Yes")), lcd.write(byte(1));}
          break;
      }
      break;
    // --- MODO MIDI CLOCK ---
    case 4:
      ...
      else if(MidiProgramming::modeMidiClock == 1) {writeAt(4, 1, byte(0)), lcd.print(F("Internal"));}
      break;
    ...
    case 5:
      ...
      switch(menusUI.seleccion){
        case 1:
          ...
          printAt(0, 2, " "), printAt(12, 2, charEscalasNotas[s->noteTone]), lcd.print(F(" "));
          break;
        case 2:
          printAt(0, 1, " "), lcd.print(nombreEscalas[s->escalaSeleccionada]), lcd.print(F(" "));
    ...
    case 6:
      ...
      break;
    case 8: // Suponiendo que nScreen = 8 o el case correspondiente
      nScreen = 8;
      lcd.clear();

      // 1. Comprobamos si el modo extensión está activado
      // (Uso s->extensionArmed basándome en tu estructura de midiPresets.h)
      bool mostrarExtension = menusUI.editExtension; 

      for (byte i = 0; i < N_MAX_STEPS; i++) { 
        // 2. La matemática mágica de posicionamiento para 20x4
        byte coordX = 5 * (i % 4);  // Columnas: 0, 5, 10, 15
        byte coordY = i / 4;        // Filas: 0, 1, 2, 3 (gracias a la división entera)

        lcd.setCursor(coordX, coordY);

        // 3. Apuntamos al paso correcto según el botón de extensión
        Step pasoActual = mostrarExtension ? s->ext_steps[i] : s->steps[i];

        // 4. Imprimimos la nota y su octava calculada
        lcd.print(charEscalasNotas[pasoActual.note]);
        lcd.print(int(pasoActual.note / 12) + pasoActual.octave);
        
        // Rellenamos con espacios para que cada celda mida exactamente 5 caracteres
        // Evita que si una nota mide menos (ej: "Do4" vs "Fa#4") queden letras fantasma
        lcd.print(F("  ")); 
      }
      break;
    case 9:
...
inline void DrawMenus::staticScreenSettings(){
  if (juegoCaracteresCargado != 1) {
    charactersMenu1();
    juegoCaracteresCargado = 1;
  }
  lcd.clear();
  if(nScreen == 20){
    ...
    printAt(2,3,"Ver Secuencia");
  }
  else if(nScreen == 22){
    printAt(2,0,"USB Sync");
  }
  else if(nScreen == 2){
    printAt(1, 2, "Save: ");
  }
    ...
    printAt(8, 0, "N"), lcd.print((char) 223), lcd.print(F(" SEQ"));
    lcd.setCursor(0,1);
    for(int i = 0; i < N_MAX_SEQS; i++){
      lcd.print(F("  ")), lcd.print(i), lcd.print(F(" "));
    }
    printAt(8, 2, "N"), lcd.print((char) 223), lcd.print(F(" CC"));
    for(int i = 0; i < N_MAX_SEQS; i++){
