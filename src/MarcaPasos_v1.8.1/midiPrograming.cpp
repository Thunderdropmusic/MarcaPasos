...
void MidiProgramming::CCSend(){
  ...
  if (nClockMsg == 0){
    nMsgCC[id] = 0;
    int valorInicial = p->nSequence[id].steps[0].ccValue;
    int valorActual = p->nSequence[id].steps[nStep].ccValue;
    int subdivisionActual = subdivision;
    if(nStep + 1 < p->nSequence[id].nTotalSteps){
      int valorSiguiente = p->nSequence[id].steps[nStep + 1].ccValue;
      int ccStepSize = (valorSiguiente  - valorActual);
      for(int i = 0; i < subdivision; i++){
        CCinterpolation[i] = p->nSequence[id].steps[nStep].ccValue + ccStepSize * ccCurveFunction(i, subdivisionActual, p->nSequence[id].steps[nStep].ccSmoothCurve)/255;
      }
    }
    else{    
      int ccStepSize = (valorInicial - valorActual);
      for(int i = 0; i < subdivision; i++){
        CCinterpolation[i] =  p->nSequence[id].steps[nStep].ccValue + ccStepSize * ccCurveFunction(i, subdivisionActual, p->nSequence[id].steps[nStep].ccSmoothCurve)/255;
      }
    }
