...
extern const int tamanyoEscalas[];
extern int octava;
...
void checkPotes() {
  for(int i = 0; i < numPotes; i++){
    int indice = constrain(map(analogRead(potesPin[i]), 1018, 30, 0, tamanyoEscalas[escalaSeleccionada] - 1), 0, tamanyoEscalas[escalaSeleccionada] - 1);
