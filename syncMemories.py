...
def process_blocks(block_id):
    ...
    for block in data.get('results', []):
        ...
        if b_type == 'paragraph':
            ...
            hashes = "#" if b_type == 'heading_1' else "##" if b_type == 'heading_2' else "###"
            line = f"{hashes} {get_text(block)}\n\n"
        elif b_type == 'code':
            ...
            line = f"* {get_text(block)}\n"
        elif b_type == 'child_page': # Caso para cuando la página es un bloque propio
            line = f"{get_text(block)}\n\n"
